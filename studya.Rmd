---
title: "Pilot study A: Application of the Parrot Sequoia multispectral camera for assessment of surface water quality"
...
Best model with 7 validation points (w2,w4,w7 were excluded)
...
lab <- read.csv("Labresultaten.csv", header = TRUE, sep = ";")

pal <- colorRampPalette(brewer.pal(9, "RdYlGn"))
Troebelheid_data <- raster("troeb_05.tif")
Troebelheid <- stack(Troebelheid_data)

mapviewOptions(basemaps = c("Esri.WorldImagery", "OpenStreetMap.DE"), layers.control.pos="topleft",na.color = "#00000000")

viewExtent

m=mapview(Troebelheid[[1]], col.regions = pal(100), na.color= "#00000000", at = seq(14, 19, 1), legend = TRUE, layer.name='Turbidity (NTU)', maxpixels =  970632)
m@map %>%
  setView(6.055838, 51.488856, zoom = 17) %>%
  addCircleMarkers(data=lab, lng = lab$graad_L, lat = lab$graad_B, label = ~lab$Id, radius = 5, popup = paste0("TSM (mg/l) =", lab$Zwev_stof, "; Turbidity (NTU) =", lab$Troebelhei),group="Validation point")%>%
addLayersControl(baseGroups = c("Esri.WorldImagery", "OpenStreetMap.DE"),overlayGroups =c("Validation point","Turbidity (NTU)"),options = layersControlOptions(collapsed = FALSE))
