...
![](ratindx_1.png){width="74%"}

![Calculated band ratio and band index reflectance maps (scale is based on specific percentile range )](ratindx_2.png){width="74%"}
...
m <- leaflet() %>%
  ...
  addProviderTiles("OpenTopoMap", group = "Topografische kaart", options=providerTileOptions(opacity = 0.6)) %>%
  addProviderTiles("Esri.WorldImagery", group = "Luchtfoto", options=providerTileOptions(opacity = 0.6)) %>%
  addCircleMarkers(data=lab, lng = lab$graad_L, lat = lab$graad_B, label = ~lab$Id, radius = 5, popup = paste0("Zwevende stof (mg/l) =", lab$Zwev_stof, "; Troebelheid (NTU) =", lab$Troebelhei)) %>%
addLayersControl(baseGroups = c("Topografische kaart", "Luchtfoto"), overlayGroups =c("NDVI","NDWI","NDRE"),options = layersControlOptions(collapsed = FALSE))
m
```
